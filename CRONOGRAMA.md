<table>
  <caption text-align="center">Cronograma</caption>
  <tr>
    <td class="dia">Segunda</td>
    <td class="dia">Terça</td>
    <td class="dia">Quarta</td>
    <td class="dia">Quinta</td>
    <td class="dia">Sexta</td>
  </tr>
  <tr>
    <td>Londrinense Tech</td>
    <td>Algoritmos de matrizes</td>
    <td>Pensamento Computacional</td>
    <td>Algoritmos de matrizes</td>
    <td>Trablho do Marcelo</td>
  </tr>
  <tr>
    <td>Relatório Londrinense</td>
    <td>Algoritmos de matrizes</td>
    <td>Relatório P.C.</td>
    <td>Algoritmos de matrizes</td>
    <td>Grupo de pesquisa de IA<td>
  </tr>
  <tr>
    <td>Relatório Londrinense Tech</td>
    <td>Algoritmos de matrizes</td>
    <td>Texte de digitação</td>
    <td>Algoritmos de matrizes</td>
    <td>Grupo de pesquisa de IA</td>
  </tr>
</table>

<details>
<summary><strong>Nos Horários Vagos</strong> </summary>
<ul>
  <li>Texte de Digitação</li>
  <li>Livro de Calculo Shaum</li>
  <li>Álgebra Linear</li>
  <li>Digital Logic Sim</li>
  <li>Alimentação/Mercado</li>
</ul>
</details>
